...
value = another dict where
    ...
    python intersect.py guardTraceroutes.pickle exitTraceroutesToDestination.pickle nameOfOutPickleDict client_as asndb_file_name
...
def get_intersection(client_as, guard_ip, exit_ip, guard_troutes_dic, exit_dest_troutes_dic, asndb):
    ...
    if client_as in guard_troutes_dic and exit_as in exit_dest_troutes_dic:
        ...
        if guard_ip in guard_ip_dict:
            ...
            # # new
            # guard_as_set.add(client_as)            
            
            # dest_ip_dict should have exactly one value
            exit_as_set = dest_ip_dict.values()[0]
            # # new
...
def main():
    ...
    out_dic_fname = sys.argv[3]
    out_dic_fname += '.pickle'
    client_as = int(sys.argv[4]) # convert to int
    asndb_file_name = sys.argv[5]
