...
question by determining the number of ASes that DNS queries traverse versus the
...
numbers~\cite{ipasn}, and generated a set of traversed ASes for DNS traceroutes
...
1,000, we are now interested in the fraction of ASes that are \emph{only}
...
account for around 70\% of all traversed ASes.  However, note that this result
only applies to exit relays that do their own DNS resolution; for relays that
use a third-party resolver, what matters is the ASes that are traversed between
