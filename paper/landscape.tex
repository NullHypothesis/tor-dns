...
number of ASs that DNS queries traverse versus the
...
web sites as seen by the Alexa search engine.  For each site, we conducted two
...
numbers~\cite{ipasn}, generating both a set of traversed ASs for DNS traceroutes
($\mathcal{D}$) and a set of traversed ASs for web traceroutes
($\mathcal{W}$).  Given these two sets for each of Alexa's Top
1,000, we compute the fraction of ASs that are \emph{only}
...
account for around 70\% of all traversed ASs.  This result
only applies to exit relays that do their own DNS resolution; for relays that
use a third-party resolver, the ASs that are traversed between
...
four of the most popular ASs could observe.  Google averages at 33\%, but at
times saw more than 40\% of all DNS requests exiting the Tor network---an
