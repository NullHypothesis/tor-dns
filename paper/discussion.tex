...
In Section~\ref{sec:load-freq}, we discussed setting up an exit relay to
determine the number of DNS requests per five minute interval.  Since
our exit relay was forwarding traffic of Tor users, we contacted Princeton
...
Exit relay operators face a dilemma: they must either operate their own
...
dimensions to this.  In lieu of substantial DNS protocol improvements, we
...
However, this setup introduces the possibility of misconfigured and censored DNS
...
very centralized data sinks that Tor aims to avoid.
...
further be configured to minimize information leakage, by enabling
QNAME minimization~\cite{Bortzmeyer2016a}.  There likely is a measurable
performance difference between a local resolver and Google's resolver, but we
believe that this difference pales in comparison to other performance issues in
Tor such as head-of-line blocking.
...
alternative.  Facebook, for example, set up facebookcorewwwi.onion.  When
...
deploy defenses against these attacks.  Since \name attacks
