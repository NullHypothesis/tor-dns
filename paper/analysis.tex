...
on April 15th 2016.  However, the recall will be low as observed in
\ref{eq:hprecall}.
...
attacks).  Note that the following results largely apply also for weaker
...
to different bandwidth fractions, as shown previously.
Unless stated otherwise, we perform our evaluation on
...
In case we overestimated the current Tor network size in the analysis described
in Section~\ref{sec:load-freq}, the \name attacks would have even higher
precision, as shown by the values for scaling factors below 1.
