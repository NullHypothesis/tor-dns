...
traceroute information. In these cases, we searched for AS compromises between the ingress 
AS-level path and the AS of the exit relay (just like the \emph{ISP DNS} configuration 
above) before treating that particular stream as having not been compromised in order 
...
with on the egress side. \emph{Google DNS only} was next, and we believe this is because
 of Google's heavily \fixme{anycast/replicated} servers. 
