...
differs by around 7 days between DE and UK and around 8 days between 
DE and FR. For ``ISP DNS only,'' the median time until compromise differs by 
around 6 days between US and DE and around 5 days between US and FR.  
Also, we notice that DE fares worse than the others in the 
``Google DNS only'' scenario 
and better than the others in the ``ISP DNS only'' scenario.  
