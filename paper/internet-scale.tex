...
probes in many ASes that also have Tor relays.  

We leverage this observation by
...
and the bandwidth of Tor relays is not uniformly distributed.  In addition to
Atlas, we also considered using PlanetLab~\cite{planetlab} to initiate
traceroutes, but unfortunately most PlanetLab nodes are located in research and
education networks~\cite{Banerjee2004a}, and are thus not well-suited for
...
\begin{table}[t]
	...
	co-located with Tor guard and exit relays.}
	\topcap{\tabcaptext}
	\centering
	\begin{tabular}{l|r r}
	\toprule
