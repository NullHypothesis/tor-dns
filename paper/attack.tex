...
In April 2016, we visited the Alexa top one million websites
...
user has visited a specific website. Figure~\ref{fig:unique-domains} shows the fraction of 
sites with unique domains for groups of websites in the Alexa top one million.
For 96.8\% of all sites there exists at least one unique domain. Interestingly, more
...
top one million dataset with five samples, we consider a closed world
and an open world. In the closed world, the attacker can use samples
...
consider 433,000 unmonitored sites that are not in the original training
set.  The number of unmonitored sites is extrapolated from our power-law
distribution, giving a relastic base rate (for the entire Tor network)
for evaluating our classifier.  The open-world evaluation yields a
...
attacks by building a classifier that takes advantage of websites with known
fingerprints. 
Conceptually, training the classifier could also involve
a custom weight-learning run that trains only on observed sites, but we found
that this approach yielded little to no improvement in classifier accuracy, despite significant increases in
testing time.
