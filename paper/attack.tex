...
how this information can help determine whether a user has visited a websites of interest.
In April 2016, we visited Alexa top one million websites
and collected five samples of all DNS requests generated by visiting each
website.
We performed in rounds from a university network, where
...
identified by Khattak \ea~\cite{Khattak2016a}. We also include Google, which
reportedly restricts access to Tor users (when searching), due to
its prevalance in the dataset.
...
We collected 2,540,941 distinct domain names over 60,828,453 DNS
requests. The dataset contains 2,260,534 domains that are unique to
a particular website; we call these domains {\em unique domains}. Figure~\ref{fig:unique-domains} shows the percentage of
...
To analyze the feasability of mapping DNS requests to websites, we construct a
