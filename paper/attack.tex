...
distribution) offered significantly better fits\footnote{We used the
		...
		distribution had an $\alpha$ parameter of about $1.13$. Note
		that pageview numbers as collected by Alexa ignore multiple
...
29.1 outgoing DNS requests (assuming a power-law distribution of site
popularity as described above and taking into account that the client
may send requests for the same domain several times until it gets served
a response -- until the first response for a domain is cached at the
exit relay, this can also causes multiple outgoing DNS request from the
exit relay for the same domain). 
This means that we had an average of about 4.1 website visits per five
...
each of the about 1,200 exit relays active at that time. According to
these calculations the total number of websites visited through the Tor
network is 316.3 websites per second.
