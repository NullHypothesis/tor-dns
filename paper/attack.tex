...
For all our attacks, unless otherwise noted, we set $k=2$. Next, we propose
a number of attacks that augment Wa-kNN with DNS data, followed by
an experimental evaluation.

\subsubsection{Confirmation-based attacks}
We define three attacks that confirm the output of Wa-kNN with the observed
...
The HP attack increases false negatives but reduces the number of false
positives. The HR attack reduces confusion with one close neighbour in
Wa-kNN, reducing false negatives but increasing false positives.

\subsubsection{Close-the-world attack}
